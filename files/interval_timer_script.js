...
function audio_change() {
    ...
    $('#audio_beep source[type="audio/mp3"]').attr('src', 'files/23.mp3');
    // ставим звук на паузу и подгружаем его
    audio[0].pause();
    audio[0].load();
}

// разбиваем секунды на часы, минуты и секунды с помощью функции:
...
// функция, добавляющая первым 0, если число меньше 9: 
...
function timerTick(type, timer_params) {
    ...
    if(interval_type == 'work') {
        ...
        if(timer_params.time_rest - now_seconds > 0) {
        ...
        } else { // а если всё-таки время отдыха закончилось
                // обнуляем табло
            renderTimerNums(0);
            // проигрываем звук уведомления
            $('#audio_beep')[0].play();
            ...
            } else { // иначе, если количество пройденных повторов меньше, чем было задано
                // обновляем информацию на панели настроек
...
$('#timer_pause').click(function(event, params) {
    ...
    } else { // иначе сразу задаём параметры звука
        params = {audio: 1};
    }
...
$('#timer_clear').click(function() {
    ...
    $('.timer_interval_rest .seconds').text('25');
